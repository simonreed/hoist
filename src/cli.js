...
// ── Paths ─────────────────────────────────────────────────────────────────────
// HOIST_DIR overrides ~/.share-cli for isolated test runs
...
const CF_DIR = APP_DIR; // config.yml lives in APP_DIR when overridden
const CONFIG_FILE = join(CF_DIR, "config.yml");
...
function writeConfig(state) {
  ...
  const credentialsFile = join(CF_DIR, `${state.tunnelId}.json`);

  if (state.mappings.length === 0) {
    // Minimal config with no real ingress — tunnel will start but serve 404
    const yaml = `tunnel: ${state.tunnelId}
...
function ensureDns(state, subdomain) {
  ...
  if (result.status !== 0) {
    ...
    const isOwnRecord = (output.includes("already") || output.includes("exists")) && !output.includes("different");
    if (isOwnRecord) {
      console.log("exists");
    } else {
      console.log("failed");
...
function startTunnel(state) {
  ...
  // Stop any existing instance, then give it a moment to die
  stopTunnel();
...
function resolveTunnelId(tunnelName) {
  const result = spawnSync(
    CLOUDFLARED,
  ...
  let tunnels;
  try {
    tunnels = JSON.parse(result.stdout);
  } catch {
  ...
  const tunnel = tunnels.find(
    (t) => t.name === tunnelName || t.id === tunnelName
  );
...
function cmdInit([domain, tunnelName]) {
  ...
  const state = { domain, tunnelName, tunnelId, mappings: [] };
  saveState(state);
  writeConfig(state);
...
function cmdShare([subdomain, portRaw]) {
  ...
  if (TEST_MODE) return; // skip interactive watch in automated tests

  // Brief pause for tunnel to connect, then hand off to watch
  spawnSync("sleep", ["2"]);
  cmdWatch({ filterHost: `${subdomain}.${state.domain}`, stopOnExit: true });
}

function cmdRm([subdomain]) {
  if (!subdomain) {
...
function cmdLs() {
  ...
  if (state.mappings.length === 0) {
    ...
    const maxLen = Math.max(...state.mappings.map((m) => m.subdomain.length + state.domain.length + 1));
...
function cmdStop() {
  ...
  if (stopped) {
    ...
    console.log("Tunnel was not running.");
  }
}
...
function parseLogLine(line) {
  // Timestamp is always the first token: 2026-03-19T10:16:24Z
...
function cmdWatch({ filterHost = null, stopOnExit = false } = {}) {
  const state = requireState();
  ...
  const tail = spawn("tail", ["-f", "-n", "200", LOG_FILE], { stdio: ["ignore", "pipe", "ignore"] });
  ...
  rl.on("line", (line) => {
    ...
    if (event.type === "req") {
      ...
      const path = pending.path.length > 40
        ? pending.path.slice(0, 37) + "..."
        : pending.path.padEnd(40);
      const status = `${color}${BOLD}${event.status}${RESET}`;
      const host = `${DIM}${pending.host}${RESET}`;
      console.log(`  ${DIM}${event.time}${RESET}  ${method} ${path}  ${status}  ${host}`);
      pending = null;
    }
  });
...
function cmdLogs() {
  ...
  const tail = spawn("tail", ["-f", "-n", "500", LOG_FILE], { stdio: ["ignore", "pipe", "ignore"] });
  const rl = createInterface({ input: tail.stdout });
  ...
  rl.on("line", (line) => {
    ...
    if (event.type === "req") {
      ...
      const status = `${color}${BOLD}${event.status}${RESET}`;
      const host = `${DIM}${pending.host}${RESET}`;
      console.log(`${event.time}  ${method} ${pending.path}  ${status}  ${host}`);
      pending = null;
    }
  });
...
const COMMANDS = new Set(["init", "add", "rm", "ls", "logs", "watch", "status", "stop", "run", "help"]);

if (!first) {
  printHelp();
} else if (!COMMANDS.has(first)) {
  // hoist <subdomain> <port>  — default command; cmdShare validates args
  cmdShare([first, rest[0]]);
} else {
  switch (first) {
    case "init":    cmdInit(rest);   break;
    case "add":     cmdShare(rest);  break;  // alias
